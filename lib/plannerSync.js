...
export function removeProjectPlannerEntries(currentEntries, projectId) {
  const target = String(projectId || '');
  return (Array.isArray(currentEntries) ? currentEntries : []).filter(entry => !(
    entry?.source === 'project' && String(entry?.projectId || '') === target
  ));
}
