...
let azureCredential;
let poolAuthenticationMode = null;
let poolTokenExpiresOnTimestamp = 0;

const TOKEN_RENEWAL_WINDOW_MS = 5 * 60 * 1000;

function getAzureCredential() {
  if (azureCredential) return azureCredential;
  const runsInAzureAppService = Boolean(process.env.IDENTITY_ENDPOINT || process.env.MSI_ENDPOINT);
  azureCredential = runsInAzureAppService
    ? new ManagedIdentityCredential(process.env.AZURE_CLIENT_ID || undefined)
...
async function getAccessToken() {
  const now = Date.now();
  if (tokenCache.token && tokenCache.expiresOnTimestamp - now > TOKEN_RENEWAL_WINDOW_MS) return tokenCache;
...
async function config() {
  ...
  if (sqlUser && sqlPassword) {
    poolAuthenticationMode = 'sql-credentials';
    poolTokenExpiresOnTimestamp = 0;
    return { ...common, user: sqlUser, password: sqlPassword };
  }

  const accessToken = await getAccessToken();
  poolAuthenticationMode = 'managed-identity';
  poolTokenExpiresOnTimestamp = accessToken.expiresOnTimestamp || (Date.now() + 55 * 60 * 1000);
  return {
    ...common,
    authentication: {
...
export async function getPool() {
  if (
    poolPromise
    && poolAuthenticationMode === 'managed-identity'
    && poolTokenExpiresOnTimestamp - Date.now() <= TOKEN_RENEWAL_WINDOW_MS
  ) {
    await resetPool();
  }
  if (!poolPromise) {
...
export async function resetPool() {
  ...
  try {
    const currentPool = await currentPoolPromise;
    await currentPool?.close();
  } catch {
    // A failed connection has nothing useful to close.
  }
}

export function databaseAuthenticationMode() {
  const fromUrl = parseDatabaseUrl(process.env.DATABASE_URL);
  const sqlUser = process.env.SQL_USER || fromUrl.user;
  const sqlPassword = process.env.SQL_PASSWORD || fromUrl.password;
  return sqlUser && sqlPassword ? 'sql-credentials' : 'managed-identity';
}
...
export async function ensureSchema() {
  ...
  await pool.request().query(`
    ...
    IF COL_LENGTH('dbo.Users','PermissionsJson') IS NULL ALTER TABLE dbo.Users ADD PermissionsJson NVARCHAR(MAX) NOT NULL CONSTRAINT DF_Users_PermissionsJson DEFAULT '[]';
    IF COL_LENGTH('dbo.Users','FolderAccessJson') IS NULL ALTER TABLE dbo.Users ADD FolderAccessJson NVARCHAR(MAX) NOT NULL CONSTRAINT DF_Users_FolderAccessJson DEFAULT '{}';
    ...
    CREATE TABLE dbo.InventoryItems (
      ...
      Unit NVARCHAR(50) NOT NULL DEFAULT 'stk.',
      IssueMode NVARCHAR(50) NOT NULL DEFAULT 'none',
    ...
    CREATE TABLE dbo.InventoryTransactions (
      ...
      ChangeQuantity DECIMAL(18,3) NOT NULL,
    ...
    IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name='IX_InventoryTransactions_ItemId' AND object_id=OBJECT_ID('dbo.InventoryTransactions'))
      CREATE INDEX IX_InventoryTransactions_ItemId ON dbo.InventoryTransactions(ItemId);
  `);
  return pool;
}
