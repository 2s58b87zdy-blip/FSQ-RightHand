...
async function config() {
  ...
  const server = process.env.SQL_SERVER || fromUrl.server;
  const database = process.env.SQL_DATABASE || fromUrl.database;
  if (!server || !database) throw new Error('Azure SQL is not configured. Set SQL_SERVER and SQL_DATABASE, or provide a valid DATABASE_URL.');
