...
const inventoryRouteSource = fs.readFileSync('app/api/inventory/route.js', 'utf8');
...
assert.match(pageSource, /\['131','MIG \/ GMAW — solid wire, inert gas'\]/);
assert.match(pageSource, /131 MIG er med som første valg/);
assert.match(pageSource, /code==='131'\?'featuredProcess'/);
...
assert.match(pageSource, /Ø1\.2 mm wire \(SMO\) — exact classification from WPQR/);
assert.match(pageSource, /Add "\(SMO\)" after the filler wire text in the WPS line/);
assert.match(pageSource, /className=\{`wpsInlineStatus/);
assert.match(pageSource, /async function deleteCompanyDocument\(document\)/);
assert.match(pageSource, /method:'DELETE'/);
assert.match(pageSource, /companyFileDelete/);
assert.match(pageSource, /Modtag nyt materiale/);
assert.match(pageSource, /Bekræft varemodtagelse/);
assert.match(pageSource, /function receiveNewItem/);
assert.match(inventoryRouteSource, /body\.action === 'receive'/);
assert.match(inventoryRouteSource, /body\.action === 'receive_new'/);
assert.match(inventoryRouteSource, /Varemodtagelse/);
assert.match(inventoryRouteSource, /Ny varemodtagelse/);
assert.match(pageSource, /en sikker WPS-kladde er oprettet/);
assert.match(wpsRouteSource, /function safeFallbackValues/);
assert.match(wpsRouteSource, /FILLER_DIAMETER = '1\.2 mm \(SMO\)'/);
assert.match(wpsRouteSource, /responseRequest\.reasoning = \{ effort: 'low' \}/);
assert.match(wpsRouteSource, /fallback: Boolean\(fallbackReason\)/);
...
assert.match(approvalsSource, /Kun Flemming og Jakob kan godkende/);
assert.match(approvalsSource, /Godkend slutkontrol og afslut job/);
assert.match(myJobsSource, /function submitFitup/);
assert.match(approvalsSource, /function approveFitup/);
assert.match(approvalsSource, /fitupPending/);
assert.match(pageSource, /consolidateAutomaticJobs/);
assert.match(pageSource, /Pending fit-up/);
