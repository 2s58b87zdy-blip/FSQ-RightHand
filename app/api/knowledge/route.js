...
export async function POST(request){
  try{
    ...
    const blobName=`knowledge/${folder}/${machine}/${Date.now()}-${name}`;const c=container();await c.createIfNotExists();const block=c.getBlockBlobClient(blobName);const buffer=Buffer.from(await file.arrayBuffer());
