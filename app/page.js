...
function Projects({projects,setProjects,deletedProjects,setDeletedProjects,setActive,setActiveProjectId}) {
  ...
  function normalizeProject(project){return {lifecycle:project.lifecycle||(project.status==='Completed'?'Completed':'Active'),archivedAt:project.archivedAt||'',completedAt:project.completedAt||'',...project}}
  const normalizedProjects=projects.map(normalizeProject);
  function add(){if(!form.name.trim()||!form.customer.trim())return;const project={id:Date.now(),...form,lifecycle:'Active',archivedAt:'',completedAt:'',progress:5,next:'Planning',projectNo:form.projectNo||`FSQ-${String(Date.now()).slice(-5)}`};setProjects([...normalizedProjects,project]);setShowWizard(false);setForm({customer:'',name:'',imo:'',projectNo:'',lead:'Flemming',location:'',startDate:'',mobilisation:'',status:'Planning',notes:''});setActiveProjectId(project.id);setActive('projectHub')}
  ...
  <div className="projectFilters">{['Active','Completed','Archived','All'].map(x=><button key={x} className={filter===x?'active':''} onClick={()=>setFilter(x)}>{x}<span>{x==='All'?normalizedProjects.length:normalizedProjects.filter(p=>p.lifecycle===x).length}</span></button>)}<button className={filter==='Trash'?'active trashFilter':''} onClick={()=>setFilter('Trash')}>Trash<span>{deletedProjects.length}</span></button></div>
