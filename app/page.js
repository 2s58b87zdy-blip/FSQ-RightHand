...
const DEFAULT_PROJECTS = [
  { id: 1, name: 'Wind Orca', customer: 'Cadeler', imo: '9671414', projectNo: 'FSQ-26071', status: 'Fabrication', progress: 68, lead: 'Jakob', location: 'Esbjerg', startDate: '2026-07-15', mobilisation: '2026-07-20', next: 'Complete SMO fabrication', notes: 'Priority scrubber modification project.' },
  { id: 2, name: 'Wind Osprey', customer: 'Cadeler', imo: '9671426', projectNo: 'FSQ-26072', status: 'Planning', progress: 42, lead: 'Flemming', location: 'Esbjerg', startDate: '2026-07-18', mobilisation: '2026-07-27', next: 'Finalize material package', notes: 'Prepare fabrication and mobilisation package.' },
  { id: 3, name: 'TORM Splendid', customer: 'TORM', imo: '9683572', projectNo: 'FSQ-26073', status: 'Inspection', progress: 27, lead: 'Jakob', location: 'Rotterdam', startDate: '2026-07-10', mobilisation: 'In progress', next: 'Finish inspection report', notes: 'Scrubber inspection and repair scope.' }
];
  ...
  return (
    <div className="appShell">
      ...
      <main className="workspace">
        ...
        {active === 'projects' && <Projects projects={projects} setProjects={setProjects} setActive={setActive} setActiveProjectId={setActiveProjectId} />}
...
function Projects({projects,setProjects,setActive,setActiveProjectId}) {
  const [showWizard,setShowWizard]=useState(false);
  const [form,setForm]=useState({customer:'',name:'',imo:'',projectNo:'',lead:'Flemming',location:'',startDate:'',mobilisation:'',status:'Planning',notes:''});
  function update(field,value){setForm({...form,[field]:value})}
  function add(){if(!form.name.trim()||!form.customer.trim())return;const project={id:Date.now(),...form,progress:5,next:'Planning',projectNo:form.projectNo||`FSQ-${String(Date.now()).slice(-5)}`};setProjects([...projects,project]);setShowWizard(false);setActiveProjectId(project.id);setActive('projectHub')}
  function openProject(id){setActiveProjectId(id);setActive('projectHub')}
  return <div className="content"><div className="sectionIntro projectIntro"><div><h1>Marine Projects</h1><p>Open a Project Hub or create a new project workspace.</p></div><button className="primaryBtn" onClick={()=>setShowWizard(!showWizard)}>{showWizard?'Close':'New Project'}</button></div>
  {showWizard&&<section className="panel projectWizard"><div className="wizardGrid"><label>Customer<input value={form.customer} onChange={e=>update('customer',e.target.value)}/></label><label>Vessel / project<input value={form.name} onChange={e=>update('name',e.target.value)}/></label><label>IMO<input value={form.imo} onChange={e=>update('imo',e.target.value)}/></label><label>Project number<input value={form.projectNo} onChange={e=>update('projectNo',e.target.value)}/></label><label>Project manager<select value={form.lead} onChange={e=>update('lead',e.target.value)}><option>Flemming</option><option>Jakob</option><option>Stefan</option></select></label><label>Location<input value={form.location} onChange={e=>update('location',e.target.value)}/></label><label>Start date<input type="date" value={form.startDate} onChange={e=>update('startDate',e.target.value)}/></label><label>Mobilisation<input type="date" value={form.mobilisation} onChange={e=>update('mobilisation',e.target.value)}/></label><label>Status<select value={form.status} onChange={e=>update('status',e.target.value)}><option>Planning</option><option>Fabrication</option><option>Inspection</option><option>Mobilisation</option><option>Completed</option></select></label><label className="wizardNotes">Notes<textarea value={form.notes} onChange={e=>update('notes',e.target.value)}/></label></div><div className="wizardFolders">{['Quotations','Purchase Orders','Drawings','Method Statements','Risk Assessments','Service Reports','WPQR','WPS','NDT Reports','Certificates','Packing Lists','Photos','Videos','Archive'].map(x=><span key={x}>{x}</span>)}</div><button className="primaryBtn" onClick={add}>Create Project Hub</button></section>}
  <div className="projectCards projectHubCards">{projects.map(p=><article key={p.id} onClick={()=>openProject(p.id)}><div className="projectTop"><div className="projectBadge">{p.status}</div><span>{p.projectNo||'No project no.'}</span></div><h3>{p.name}</h3><p>{p.customer} · {p.location||'TBD'}</p><div className="projectFacts"><span>IMO {p.imo||'N/A'}</span><span>Lead {p.lead}</span></div><div className="progress big"><span style={{width:`${p.progress}%`}}/></div><div className="meta"><span>{p.progress}% complete</span><span>{p.mobilisation||'Mobilisation TBD'}</span></div><p className="nextLine">Next: {p.next}</p><button className="openHub">Open Project Hub</button></article>)}</div></div>
}

function ProjectHub({project,projects,setProjects,people,tasks,setTasks,documents,materials,quotes,reports,setActive}) {
  ...
  return <div className="content projectHub"><button className="backButton" onClick={()=>setActive('projects')}>← Back to projects</button><section className="projectHero"><div><div className="projectHeroTop"><span className="projectBadge">{project.status}</span><span>{project.projectNo}</span></div><h1>{project.name}</h1><p>{project.customer} · IMO {project.imo||'N/A'} · {project.location||'TBD'}</p></div><div className="projectHeroStats"><div><strong>{project.progress}%</strong><small>Progress</small></div><div><strong>{projectTasks.filter(t=>t.status!=='Completed').length}</strong><small>Open tasks</small></div><div><strong>{projectDocs.length}</strong><small>Documents</small></div><div><strong>{crew.length}</strong><small>Crew</small></div></div></section><nav className="projectTabs">{tabs.map(([id,label])=><button key={id} className={tab===id?'active':''} onClick={()=>setTab(id)}>{label}</button>)}</nav>
  {tab==='overview'&&<section className="hubGrid"><div className="panel"><h3>Project details</h3><div className="detailGrid"><label>Status<select value={project.status} onChange={e=>update('status',e.target.value)}><option>Planning</option><option>Fabrication</option><option>Inspection</option><option>Mobilisation</option><option>Completed</option></select></label><label>Project manager<select value={project.lead} onChange={e=>update('lead',e.target.value)}><option>Flemming</option><option>Jakob</option><option>Stefan</option></select></label><label>Location<input value={project.location||''} onChange={e=>update('location',e.target.value)}/></label><label>Mobilisation<input value={project.mobilisation||''} onChange={e=>update('mobilisation',e.target.value)}/></label><label>Progress {project.progress}%<input type="range" min="0" max="100" value={project.progress} onChange={e=>update('progress',Number(e.target.value))}/></label><label>Next milestone<input value={project.next||''} onChange={e=>update('next',e.target.value)}/></label></div></div><div className="panel"><h3>Readiness</h3>{[['Project plan',true],['Drawings',projectDocs.some(d=>d.category==='Drawings')],['Method Statement',projectDocs.some(d=>d.category==='Method Statements')],['Risk Assessment',projectDocs.some(d=>d.category==='Risk Assessments')],['Certificates',projectDocs.some(d=>d.category==='Certificates')],['Packing List',projectDocs.some(d=>d.category==='Packing Lists')]].map(([n,ok])=><div className="checkRow" key={n}><span className={ok?'ok':'missing'}>{ok?'✓':'!'}</span><b>{n}</b><em>{ok?'Ready':'Missing'}</em></div>)}</div><div className="panel"><h3>Project notes</h3><textarea className="hubTextarea" value={project.notes||''} onChange={e=>update('notes',e.target.value)}/></div><div className="panel"><h3>Next actions</h3>{projectTasks.filter(t=>t.status!=='Completed').slice(0,5).map(t=><div className="taskMini" key={t.id}><span className={t.priority==='High'?'dot danger':'dot'}/><div><b>{t.title}</b><small>{t.person} · {t.status}</small></div></div>)}</div></section>}
  {tab==='crew'&&<section className="panel"><div className="peopleCards">{crew.length?crew.map(p=><article key={p.id}><div className="personHead"><div className="avatar">{p.name[0]}</div><div><h3>{p.name}</h3><small>{p.location} · {p.status}</small></div></div><p>{p.task}</p></article>):<div className="empty">Assign crew in Crew Management.</div>}</div></section>}
